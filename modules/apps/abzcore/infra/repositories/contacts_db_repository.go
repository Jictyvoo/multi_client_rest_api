...
import (
	"database/sql"
	"errors"
	"github.com/jictyvoo/multi_client_rest_api/modules/apps/abzcore/infra/models"
...
func (repo ContactsDbRepository) Add(dto interfaces.ContactDTO) error {
	const sqlCmd = `INSERT INTO contacts (nome, celular) VALUES (?, ?)`
...
func (repo ContactsDbRepository) AddAll(dto []interfaces.ContactDTO) (err error) {
	...
	for _, contact := range dto {
		_, err = tx.Exec(`INSERT INTO contacts (nome, celular) VALUES (?, ?)`, contact.Name(), contact.Phone())
		if err != nil {
			return err
		}
	}
...
func (repo ContactsDbRepository) GetByPhone(s string) (interfaces.ContactDTO, error) {
	const sqlCmd = `SELECT nome, celular FROM contacts WHERE celular = ?`
	var contact models.ContactsModel
